...
$selectdata = "SELECT `model`.`ModelID` AS MID, `model`.`ModelNumber` AS MNUM, `device`.`Name` AS DNM FROM `model` INNER JOIN `device` ON `model`.`DeviceID` = `device`.`DeviceID` WHERE `ModelNumber` LIKE '%".$_POST['searchText']."%' LIMIT 10";
